...
export default function TableView({
  ...
  // Ordenar filas por el tiempo más pequeño que tengan (ascendente)
  const sortedRows = React.useMemo(() => {
    const getMinTimeForRow = (rowObj) => {
      ...
      const valid = times.filter((t) => t != null && !Number.isNaN(t));
      return valid.length ? Math.min(...valid) : Infinity;
    };

    return [...filteredRows].sort((a, b) => {
      const ma = getMinTimeForRow(a[1]);
      const mb = getMinTimeForRow(b[1]);
      if (ma === mb) return 0;
      // rows with no times (Infinity) go to the end
      if (ma === Infinity) return 1;
      if (mb === Infinity) return -1;
      return ma - mb;
    });
  }, [filteredRows, components]);
    ...
    <>
      ...
      <div style={{ overflowX: "auto", marginTop: 8 }}>
        <table className="sim-table">
          ...
          <tbody>
            {sortedRows.map(([elemName, rowObj]) => {
